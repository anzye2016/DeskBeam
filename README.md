...
```
录音 → server.py ──┬── asr_api_url 不为空 → server.py 直接调在线 API
                   └── 走本地 → WSL → asr.py → 本地模型服务 (:8082)
```

**在线 API**：内置在 `server.py` 中，配置 `config.json` 即可，无外部依赖。

**本地 WSL**：由外部脚本 `asr.py` 处理，项目代码不关心模型实现细节。`asr.py` 负责把 WAV 音频转发到本地 ASR 模型服务（默认 `127.0.0.1:8082`），换模型只需改 `asr.py` 里的 `SERVER` 地址，项目代码无需改动。
