...
支持两种方式：在线 API 或本地 WSL ASR。**在线优先**（配置了 API 密钥就走在线）。
...
兼容 OpenAI 格式的语音识别接口（如小米 MiMo ASR）。音频以 base64 上传，返回文本自动输入到当前窗口。

### 方式二：WSL 本地模型

依赖 WSL 和一个运行的 ASR 模型服务（默认端口 8082）。项目自带 `asr.py` 脚本：

```bash
# 在 WSL 中部署
cp asr.py ~/scripts/
# 启动 ASR 模型服务在 127.0.0.1:8082
```
