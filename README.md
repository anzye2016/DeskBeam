...
## 4. Remote access via SSH tunnel / 远程访问（SSH 隧道）

Access DeskBeam from the internet through a cloud relay server. | 通过云服务器中转，从外网访问 DeskBeam。

```
Browser ──TLS(LE)──→ Cloud Nginx ──SSH──→ Local DeskBeam
```

### Server setup / 服务端配置 (Linux)
...
server {
    ...
    location / {
        proxy_pass http://127.0.0.1:18769;  # or https:// + proxy_ssl_verify off
...
### Windows tunnel / Windows 端隧道

```powershell
# Start tunnel / 启动隧道
.\tunnel.ps1 start

# Daemon with auto-reconnect / 守护进程（断线自连）
.\tunnel-daemon.ps1
...
See `tunnel.ps1` / `tunnel-hd.ps1` for server-specific configs (key file, port). | 不同服务器对应各自的 `tunnel-*.ps1` 脚本。

---

## 5. Run as background service / 后台运行
...
## 7. Browser compatibility / 浏览器兼容性
